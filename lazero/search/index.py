...
def retrieveFilePathFromLineIndex(line_index, databasePath=os.path.join(lazeroCachePath, "index.json")):
...
def retrieveLineRangeFromFilePath(filepath, databasePath=os.path.join(lazeroCachePath, "index.json")):
    # line index need not to be normalized.
    User, database = tinydbQueryBootstrap(databasePath)
...
def indexFilesInDirectory(
    ...
    for absoluteFilePath in listFilesInDirectory(directory):
        ...
        total_length = len(listOfCleanedMergedConvGroupWithLineIndexMapping) * divisor # because we use the original line.
        ...
        for index in range(total_length):
            ...
            line_range = listOfCleanedMergedConvGroupWithLineIndexMapping[index//divisor][
                "line_range"
            ]
            mIndex = start + index
            ...
            temp_processed_line_index_mapping.append( # here's how we store the original conv_group content
                (str(mIndex//divisor)+"_content", linewise[index//divisor])
            )
        ...
        for changed_lines in getFourVersionOfLineInListOfCleanedMergedConvGroupWithLineIndexMapping(
            listOfCleanedMergedConvGroupWithLineIndexMapping, withOriginalLine=withOriginalLine
        ):
